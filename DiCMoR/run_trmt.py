...
# missing_weights：欠損重み

def DICMOR_run_Missing_Weights(
        model_name, dataset_name, config=None, config_file="", seeds=[], mr=0.1, is_tune=False,
        tune_times=500, feature_T="", feature_A="", feature_V="",
        model_save_dir="./pt", res_save_dir="./result", log_dir="./log",
        gpu_ids=[0], num_workers=4, verbose_level=1, mode='train', missing_weights=None
):
    # Initialization
    model_name = model_name.lower()
    dataset_name = dataset_name.lower()
    ...
    #========================================================== ↓↓↓ added by trmt on 24/4/4
    args.missing_weights=missing_weights
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    epoch_results = trainer.do_train(model, dataloader, return_epoch_results=from_sena)
    model.load_state_dict(torch.load(args.model_save_dir+'/dicmor-{}.pth'.format(args.dataset_name)))
    # model.load_state_dict(torch.load('pt/dicmor-{}.pth'.format(args.dataset_name)))
